...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
 ...
 // extern 声明外部变量或方法,在一个文件中想引用另外一个文件中已经定义的外部变量时,则只需要在引用变量的文件中用extern 关键字加以声明即可.

  extern void JDK_Version_init();// JDK版本的初始化,里面会对版本号格式以及最小版本值做校验.

  // Check version 目前支持 1.2 / 1.4/1.6
  if (!is_supported_jni_version(args->version)) return JNI_EVERSION;

  // Initialize the output stream module
  // 里面定义了defaultStream流,并且将时间更新(主要是为了方便GC时间的统计).
  ostream_init();

  // Process java launcher properties.
  // 处理参数 sun.java.launcher sun.java.launcher.pid , 之所以单独处理这两个参数,是因为后面其他的参数可能依赖它们.
  Arguments::process_sun_java_launcher_properties(args);

  // Initialize the os module before using TLS
  // 在参数解析之前做的初始化.
  // 代码在 hotspot/src/os/linux/vm/os_linux.cpp
  // 主要做了进程号/时钟/物理内存页/互斥锁的初始化
  os::init();
...
// 开启大页支持
  os::init_before_ergo();
// 根据机器的内存/CPU等硬件信息,初始化了一些JVM内存参数/GC策略/根据CPU不同选择不同策略的BiasedLocking(偏向锁)等内容. 
  ...
  // 参数解析之后的初始化~ 主要包含: 重新加载library/ 设置栈大小/ 设置信号量及其对应的处理handle
  ...
  // 运行在单线程之下,此时是在工作线程启动之前创建native线程,去记录内存活动.
  // 这是启动的第一个阶段, 当前VM工作在单线程模式下.
  ...
  // 如果有-Xrun , -agentlib 参数,则需要执行init
  if (Arguments::init_libraries_at_startup()) {
    convert_vm_init_libraries_to_agents();
  }

  // Launch -agentlib/-agentpath and converted -Xrun agents
  // 如果有-agentlib/-agentpath 的参数,则init
  ...
  // 初始化全局数据结构, 并且在堆中创建系统类
  // 主要涉及到 当前线程是否有没有处理的异常/基础类型的初始化/原子锁的初始化/内存池的初始化
  vm_init_globals();

  // Attach the main thread to this os thread
  JavaThread* main_thread = new JavaThread();// 主线程
  main_thread->set_thread_state(_thread_in_vm); // 设置主线程的状态 running in vm
  ...
  // 在threadlocalstorage 中初始化主线程的内存
  ...
  // guard page 是提供内存访问一次性警报的机制.用来提供对动态增长的数据结构的一种监控机制.比如分配了若干长度的一个数据结构,然后设置紧随其后的一页为guard page. 当访问该数据结构越界,踩到该guard page 时,就会触发CPU 报警,并调用事先操作系统定义好的警报处理程序做处理.比如分配了若干长度的一个数据结构,然后设置紧随其后的一页为guard page. 当访问该数据结构越界,踩到该guard page 时,就会触发CPU 报警,并调用事先操作系统定义好的警报处理程序做处理.
  main_thread->create_stack_guard_pages();

  // Initialize Java-Level synchronization subsystem
  // 初始化一个java 级别的同步子系统. 方法里面包含了JVM 性能统计相关的一些内容.
  ObjectMonitor::Initialize() ;// ./share/vm/runtime/objectMonitor.cpp
  ...
  // Create the VMThread 创建VM线程, 上面是创建java 线程
  { TraceTime timer("Start VMThread", TraceStartupTime);
	  // 创建了一个VMThread
    VMThread::create(); // ./share/vm/runtime/vmThread.cpp 里面
