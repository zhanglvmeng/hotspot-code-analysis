...
main(int argc, char ** argv)
{
    ...
    int ret;
    InvocationFunctions ifn;//struct 里面包含了CreateJavaVM_t, GetDefaultJavaInitArgs_t .
    jlong start, end;
    // 最大2048
    char jrepath[MAXPATHLEN], jvmpath[MAXPATHLEN];
    char ** original_argv = argv;
    // 如果想获取更丰富的调试信息，可以在/etc/profile 中增加 export _JAVA_LAUNCHER_DEBUG = 1 
     ...
     *     bugid 5030265 below.)
     */
    // 找到合适的JRE,并检测该JRE 是否好用.以及对参数做了一些校验.
    ...
    // 创建执行环境: 进行一些基础设置: jvm/jre/LD_LIBRARY_PATH 等. 
    ...
    // 增加3个vm 选项供javac 使用.如果用户输入这3个参数,则使用用户指定的参数.
...
// 清理内存
    ...
    if (threadStackSize == 0) {
      struct JDK1_1InitArgs args1_1;// 便于launcher获取stack 大小
...
SelectVersion(int argc, char **argv, char **main_class)
{
    ...
    manifest_info info; // manifest 文件的结构.可以参见 jdk/src/share/bin/manifest_info.h
    /**
     *typedef struct manifest_info {xxxx  }
     */
    ...
    *new_argp = NULL;
     ...
     * this data around.
     */
    // jar 文件的处理, jar 文件里面必须有manifest 信息
    if (jarflag && operand) {
	    // 解析manifest 文件, 并将manifest-version,main-class,jre-version,jre-restrict-search,splashscreen-image 等变量写入到info中 具体代码 ./jdk/src/share/bin/parse_manifest.c
     ...
     */
    // 如果jre 版本不存在,则释放manifest 以及 有关参数的内存
     ...
     */
    // JLI_ValidVersionString 主要是检查versiont字符串是否合法,比如不能为空,不能包含一些特殊的字符.
    ...
    // 检测JRE 是否好用: 首先找到jre,然后新开一个线程去检测jre是否好用~
...
TranslateApplicationArgs(int *pargc, char ***pargv)
{
    ...
    *pargv = nargv;
    ...
    // -J选项将选项传给javac 调用的java 启动器. 用-J将选项传给执行用java编写的应用程序的虚拟机是一种公共约定.
    ...
    // -classpath 类路径设置用户类路径,他将覆盖CLASSPATH环境变量中的用户类路径.
    for (i = 0; i < argc; i++) {
        char *arg = argv[i];
        if (arg[0] == '-') {
            if (arg[1] == 'J')
 ...
 * mechanism.
 */
// 读取jvm.cfg 并且填充knownJVMs 数组
// jvm.cfg 文件的目的是在测试环境下,在独立JRE安装环境的子目录下,安装几个jvm 类库.方便又节省空间.
// 目的并没有明确提供一个全面的aliasing 或者断言机制.
jint
