...
void os::Linux::initialize_system_info() {
  set_processor_count(sysconf(_SC_NPROCESSORS_CONF));// 已配置处理器数
...
void os::Linux::clock_init() {
  ...
  // dlopen 用于打开动态链接库中的函数. 
  // RTLD_LAZY : 在dlopen返回前,对于动态库中存在的未定义的变量不执行解析.就是不解析这个变量的地址.
  // 除了 RTLD_LAZY 之外,还有RTLD_NOW/RTLD_GLOBAL.
...
//  first_hrtime = gethrtime();
  ...
  // sysconf 用于获得与文件或目录无关的限制值,以及系统特征选项. 
  // _SC_CLK_TCK 的返回值是每秒钟的时钟滴答数,以用于times函数的返回值.
  clock_tics_per_sec = sysconf(_SC_CLK_TCK);
 // 设置random 的seed
  init_random(1234567);
//ThreadCritical 是有关线程安全的类,这个方法是创建了一个线程锁.
  ThreadCritical::initialize();
// sysconf 的用途是用来获取系统执行的配置信息.比如下面的页大小.
// 设置页大小.
  ...
  // 初始化系统信息: 主要包括进程id 是否正常/ 计算物理内存大小/ 检查CPU核数. 
  Linux::initialize_system_info();

  // main_thread points to the aboriginal thread
  // pthread_self 返回的是同一进程中各个线程之间的标示号,对于这个进程内是唯一的,而不同进程中,每个线程返回的pthread_self 可能返回的是一样的.
  // 线程可以通过pthread_self 获得自己的ID.
...
// 初始化系统时钟
  ...
  // monotonic clock 翻译为单调时间,是指系统之后流逝的时间. 
  // 相对的有wall time 这个属性,其字面意思是挂钟时间,实际上是指现实时间~
  int status;
