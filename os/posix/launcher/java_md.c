...
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    // 底层就是调用了Thread::create_vm 来创建VM
    ifn->CreateJavaVM = JNI_CreateJavaVM;// 调用JNI的代码创建VM. 代码地址: jdk7u/hotspot/src/share/vm/prims/jni.cpp 
    // JNI_GetDefaultJavaVMInitArgs 主要是对参数做了校验,比如args->version~
...
LocateJRE(manifest_info* info)
{
    ...
    if (info->jre_restrict_search)
        path = JLI_StringDup(system_dir); // 默认 /usr/java
    ...
    while (dp != NULL) {
        ...
        if (cp != NULL)
            *cp = (char)NULL;
        if ((target = ProcessDir(info, dp)) != NULL)//检测路径dp中是否存在JRE
            break;
        dp = cp;
        if (dp != NULL)
            dp++;
...
// 创建线程: continuation 是线程开始执行的地方. 这个地方是传入的javaMain 函数.
    if (pthread_create(&tid, &attr, (void *(*)(void*))continuation, (void*)args) == 0) {
      void * tmp;
      pthread_join(tid, &tmp);// 子线程阻塞调用程序,知道指定的线程(tid)线程终止为止.
